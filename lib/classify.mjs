...
const widest = (list, of) => list.reduce((a, b) => (of(b) > of(a) ? b : a))
...
function describe(j, refIntrons, refExons, strand) {
  ...
  const sharesStart = refIntrons.filter(r => r.start === j.start)
  const sharesEnd = refIntrons.filter(r => r.end === j.end)
  ...
  // wrong place.
  if (sharesStart.length && sharesEnd.length) {
    return { kind: 'pairing', label: 'unannotated-pairing' }
  }
  // The donor is the intron's 5' end, which is its genomic START on the plus
  // strand and its genomic END on the minus one. Naming the two sites by
  // genomic position instead calls every minus-strand model's moved donor an
  // acceptor, and an annotator reads that as the other end of the intron.
  const atStart = strand === '-' ? 'acceptor' : 'donor'
  const atEnd = strand === '-' ? 'donor' : 'acceptor'
  if (sharesEnd.length) {
    const shift = nearest(sharesEnd, r => r.start - j.start).start - j.start
    return { kind: atStart, shift, label: `${atStart}${signed(shift)}` }
  }
  if (sharesStart.length) {
    const shift = nearest(sharesStart, r => r.end - j.end).end - j.end
    return { kind: atEnd, shift, label: `${atEnd}${signed(shift)}` }
  }
...
// GFF3 lets one exon line belong to several transcripts, which is how a shared
// first exon is written once rather than per isoform. Reading Parent whole makes
// "t1,t2" a transcript of its own and leaves both real ones an exon short.
const parents = f => (f.attrs.Parent || '').split(',').filter(Boolean)
...
function blocksByParent(features, keep) {
  const exon = new Map()
  const cds = new Map()
...
// blocksByParent collapses exon and CDS into one list, which is what the
// junction comparison wants. Rebuilding a model elsewhere needs both, and needs
// CDS phase, so this keeps them apart.
function partsByParent(features, keep) {
  const out = new Map()
  for (const f of features) {
    if ((f.type !== 'exon' && f.type !== 'CDS') || !keep(f)) {
      continue
    }
    for (const k of parents(f)) {
      if (!out.has(k)) {
  ...
  for (const list of out.values()) {
    list.sort((a, b) => a.start - b.start || a.type.localeCompare(b.type))
  }
  return out
}
...
export function classify({ predictionFile, referenceFile, refNames }) {
  ...
  for (const f of refFeatures) {
    ...
    if (f.attrs.ID) {
      txToGene.set(f.attrs.ID, parents(f)[0])
    }
  }
  ...
  // isoform at a time.
  const exonsByTx = blocksByParent(refFeatures, keep)
  ...
  const predExons = blocksByParent(predFeatures, keep)
  const predParts = partsByParent(predFeatures, keep)
            ...
            : [
                {
                  ...
                  ...describe(j, refIntrons, refExons, t.strand),
                },
              ],
        )
      : []
