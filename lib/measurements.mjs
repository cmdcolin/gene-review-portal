...
//
// The HEADER read needs this at least as much as the counts do, and used to go
// without: it runs once per BAM and every cell in that column depends on it, so
// a single blip on it emptied a whole tissue rather than one locus. That is
// exactly what one build here did.
function samtools(args, cwd, attempts = 3) {
...
function reads(bam, region, cwd) {
  const out = samtools(['view', '-c', bam, region], cwd)
...
function refNameFor(bam, refName, cwd) {
  const header = samtools(['view', '-H', bam], cwd)
  if (header === null) {
    return null
  }
  const names = new Set(
...
export function evidenceTable({ id, repro, candidates, loci, bams, names }) {
  // Every samtools call runs from a scratch directory, because htslib caches a
  // REMOTE index into the process's working directory — so counting evidence
  // over a hosted BAM drops a .bai beside whoever's shell ran the build. Same
  // trap as tabix and the .tbi; see fetchRegions.
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'portal-evidence-'))
  // Per contig, not once off the first candidate: without --region the
  // candidates span the whole assembly, and a spelling read from chr1 asks
  // samtools for `chr1:…` at every locus on every other contig. That exits
  // non-zero, is retried three times, and leaves the cell empty.
  const cache = new Map()
  const spellingFor = (bam, refName) => {
    const k = `${bam}\t${refName}`
    if (!cache.has(k)) {
      cache.set(k, refNameFor(bam, refName, cwd))
    }
    return cache.get(k)
  }
  const columns = [
  ...
  const rows = candidates.map((c, ci) => {
    ...
    const range = loci[ci].slice(loci[ci].lastIndexOf(':') + 1)
    bams.forEach((bam, i) => {
      const contig = spellingFor(bam, c.refName)
      values[`rnaseq_${i + 1}`] = contig
        ? reads(bam, `${contig}:${range}`, cwd)
        : null
    })
    return { values }
  })
